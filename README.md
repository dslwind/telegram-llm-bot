...
- SQLite persistent chat history per chat session
...
## Commands
- `/new`: clear current chat session history and start a new session
...
- `/reset`: clear current chat session history
...
- `MAX_HISTORY_PAIRS`: history message pairs loaded from the current chat session context
...
- SQLite keeps full history per chat session; `MAX_HISTORY_PAIRS` controls how much context is loaded into prompts.
